...
# TradeBot Portal - Server Setup Script
# Run this script on the GCP VM after SSH connection
# Configures IST timezone, MongoDB, Node.js, Nginx, and security
...
# Create Nginx configuration for TradeBot Portal
sudo tee /etc/nginx/sites-available/tradebot-portal > /dev/null <<EOF
# TradeBot Portal Nginx Configuration
server {
    listen 80;
...
server {
    ...
    # SSL configuration (Let's Encrypt certificates will be added later)
...
gsutil cp "$BACKUP_DIR/mongodb_$TIMESTAMP.tar.gz" gs://your-project-id-tradebot-backups/mongodb/
gsutil cp "$APP_BACKUP_DIR.tar.gz" gs://your-project-id-tradebot-backups/app/
...
# Step 11: Setup cron job for backups
...
echo ""
echo "Next Steps:"
echo "  1. Update domain DNS to point to your VM IP"
echo "  2. Run: sudo certbot --nginx -d $DOMAIN_NAME"
echo "  3. Deploy your application to $APP_DIR"
